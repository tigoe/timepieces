/*
  Creates a clock using a string of 24 neoPixels. Uses RTCZero lib for time,
...
const int numPixels = 24;    // number of pixels
...
void setHands(int h, int m, int s) {
  ...
  int hourPosition = (h) * 2;      // the outside pixels
  strip.setPixelColor(hourPosition, hourColor);
  strip.setPixelColor(m/2.5, hourColor);   // set minute pixel
  strip.setPixelColor(s/2.5, secondColor);   // set second pixel
  strip.show();
}
